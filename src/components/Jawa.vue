...
<template>
    <div class="grid grid-cols-1 md:grid-cols-3  gap-6">
        <div>
            <h4 class="text-3xl text-primary ">Group Factory List </h4>
            <transition name="slide">
                <div v-if="info" class="mt-10">
                    <div class="pl-6  border-l-4 border-primary flex flex-col gap-2">
                        <h4 class="text-lg">{{ info.title }}</h4>
        ...
        <div class="md:col-span-2">
            <div class="relative max-w-max mx-auto">
                ...
                <img src="/jawa.png" alt="" class="w-full">
            </div>
        </div>
    </div>
</template>
<style scoped>
.pin {
    @apply absolute w-5 h-5 lg:h-10 lg:w-10 rounded-full border-2 border-primary border-dashed flex justify-center items-center animate-spin-slow hover:animate-none cursor-pointer;
}
